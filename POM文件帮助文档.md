...
> 说明：本附件是字段字典，不是可以直接构建的完整 POM。大量空元素仅用于展示结构；实际项目必须删除无关项并填写必需坐标。当前示例按 Maven 3.9+/4.0.0 模型说明，敏感凭据应放在用户级 `settings.xml` 或 CI 密钥中。
...
xsi:schemaLocation="http://maven.apache.org/POM/4.0.0 http://maven.apache.org/maven-v4_0_0.xsd "> 
    ...
    <!-- modelVersion 必须位于 parent 和项目坐标之前。 -->
    ...
    <!-- 构件标识符。groupId、artifactId、version 构成基本坐标；同一版本还可能通过 type/classifier 区分附属构件。
         artifactId 通常在同一 groupId 下保持唯一。Maven 构件包括 JAR、源码包、WAR 等。 -->
    <artifactId>xxx</artifactId> 
    ...
    <issueManagement> 
        ...
        <!-- 该项目使用的问题管理系统的URL --> 
        <url> https://jira.example.com/project </url> 
    </issueManagement> 
    ...
    <mailingLists> 
        ...
        <mailingList> 
            ...
            <!-- 你可以浏览邮件信息的URL --> 
            <archive> https://example.com/demo/dev/ </archive> 
        </mailingList> 
    </mailingLists> 
    ...
    <developers> 
        ...
        <developer> 
            ...
            <!-- 项目开发者所属组织的URL --> 
            <organizationUrl> https://example.com/organization </organizationUrl> 
    ...
    <scm> 
        ...
        <connection> 
            scm:git:https://git.example.com/team/project.git
        </connection> 
        ...
        <url> https://git.example.com/team/project </url> 
    </scm> 
    ...
    <build> 
        ...
        <!-- 未在命令行指定目标时执行的默认 phase 或 plugin goal，例如 verify 或 package。phase 与 goal 不是同一概念。 -->
        ...
        <testResources> 
            ...
            <testResource> 
                <!-- 描述测试资源的目标路径。该路径相对 target/test-classes（即${project.build.testOutputDirectory}）。
                     举个例子，如果你想资源在特定的包里(org.apache.maven.messages)，你就必须该元素设置为
                    org/apache/maven/messages。然而，如果你只是想把资源放到源码目录结构里，就不需要该配置。 --> 
                <targetPath></targetPath> 
    ...
    <distributionManagement> 
        ...
        <repository> 
            ...
            <!-- Maven 3+ 对快照部署使用唯一时间戳版本，不再配置 uniqueVersion。 --> 
...
> 以下代码块都是放入 `${user.home}/.m2/settings.xml` 根元素内部的独立片段，并非一段连续 XML。完整文件需要使用 `<settings xmlns="http://maven.apache.org/SETTINGS/1.0.0" ...>` 根元素包裹。示例域名和凭据均为占位值，不可直接用于生产环境。
...
```XML
<!-- 兼容旧版 Maven 的字段。现代 Maven 项目应在 POM/pluginManagement 中锁定插件版本，不要依赖 plugin-registry.xml。 -->
...
<!--配置服务端的一些设置。一些设置如安全证书不应该和pom.xml一起分发。这种类型的信息应该存在于构建服务器上的settings.xml文件中。-->
 <servers>
  ...
  <server>
   ...
   <!--鉴权时使用的私钥位置。示例使用 Ed25519 密钥；密码可使用 Maven Password Encryption 或 CI 密钥管理，不能提交到代码仓库。 -->
...
<!--为仓库列表配置的下载镜像列表。高级设置请参阅镜像设置页面 -->
 <mirrors>
  ...
  <mirror>
   ...
   <!--该镜像的URL。example.com 是占位域名，实际使用时替换为组织内已验证的 HTTPS 仓库代理。 -->
   <url>https://nexus.example.com/repository/maven-public/</url>
   <!--被镜像的服务器的id。例如，如果我们要设置了一个Maven中央仓库（https://repo.maven.apache.org/maven2/）的镜像，就需要将该元素设置成central。这必须和中央仓库的id central完全一致。-->
...
```XML
 <!--根据环境参数来调整构建配置的列表。settings.xml 中的 profile 是 POM profile 的裁剪版本，只包含 activation、repositories、pluginRepositories 和 properties 等设置。最终值取决于 Maven 的 profile 激活与合并规则，不能笼统理解为无条件覆盖整个 POM profile。 -->
 <profiles>
  <!--根据环境参数来调整的构件的配置-->
  <profile>
   <!--该配置的唯一标识符。 -->
...
```XML
<!--手动激活 profile 的列表。每个 activeProfile 保存一个 profile id；匹配的 profile 会被激活。
        如果找不到该 id，Maven 不会因此自动创建 profile。 -->
   <activeProfiles>
